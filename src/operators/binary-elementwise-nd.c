    ...
    xnn_operator_t* binary_elementwise_op_out)
{
  ...
  binary_elementwise_op->params.f16_minmax = xnn_init_f16_minmax_params(
    ...
    size_t num_threads)
{
  ...
  binary_elementwise_op->context.elementwise_binary = (struct elementwise_binary_context) {
    ...
    .b = input2,
    .y = output,
    .elements = compressed_output_shape[0] * sizeof(uint16_t),
    .params.f16 = binary_elementwise_op->params.f16_minmax,
  };
  ...
  for (size_t i = 1; i < num_compressed_dims; i++) {
    if (compressed_a_shape[i] != 1) {
      binary_elementwise_op->context.elementwise_binary.a_stride[XNN_MAX_TENSOR_DIMS - 1 - i] = a_stride * sizeof(uint16_t);
    }
    if (compressed_b_shape[i] != 1) {
      binary_elementwise_op->context.elementwise_binary.b_stride[XNN_MAX_TENSOR_DIMS - 1 - i] = b_stride * sizeof(uint16_t);
    }
    binary_elementwise_op->context.elementwise_binary.y_stride[XNN_MAX_TENSOR_DIMS - 1 - i] = y_stride * sizeof(uint16_t);
